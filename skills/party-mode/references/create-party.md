...
A guided authoring flow that turns an idea — a themed cast, a one-off persona, or a pile of raw profile data — into custom party members and groups, written to the skill's own user override. The output is configuration: sparse `[party]` entries appended to `user/party.user.toml` (next to `party.toml` at the skill root). No external tooling — you read the current state, write the delta, and verify.

## What you're producing

Sparse `[party]` override entries for `party-mode`:

- `[[party.party_members]]` — one per persona: `code`, `name`, `icon`, `title`, `persona`, optional `capabilities`, optional `model`.
- `[[party.party_groups]]` — when the personas form a named room: `id`, `name`, an optional freeform `scene`, `members` (codes), and `memory` (`true`/`false`). `members` is optional: leave it off for an open-cast room whose `scene` names a pool the model casts from on the fly. `memory` is whether the group remembers across sessions; ask the user when they don't say, default `false`.
...
When the user wants to change a party that already exists (retune a member's persona, add someone to a group, swap the default), read the current state first so you change rather than clobber: `python3 {skill-root}/scripts/resolve_party.py --config {skill-root}/party.toml` returns the merged `party_members`, `party_groups`, and `default_party`. Show the member or group being touched, capture only the delta with the user, and write that sparse change to the user override — it replaces a `party_members`/`party_groups` entry whose `code`/`id` matches and appends the rest, so an edit is just the changed entry, never a full rewrite.
...
**First, check for code collisions.** A custom member whose `code` matches a shipped member silently *overrides* that member in the collective. Before composing, resolve the collective once — `python3 {skill-root}/scripts/resolve_party.py --config {skill-root}/party.toml` — and check each new member's `code` against the returned members. On a collision, surface it ("`craftsman` would override the shipped Yui — intended, or pick a different code?") and let the user confirm or rename. One check, not a gate.

Compose the sparse `[party]` override and append it to `user/party.user.toml` (create the file if it doesn't exist). Keep it sparse — only the new entries, never a copy of `party.toml`. If the file already has entries, make sure your additions merge cleanly by `code`/`id` (matching keys replace, new keys append) and keep the existing content intact — read it first. Then verify: re-run `resolve_party.py` and confirm the new member(s)/group(s) appear resolved in the collective.
