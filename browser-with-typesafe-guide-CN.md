...
| 执行者 | 职责 |
|--------|------|
| **宿主模型** | 任务规划、文本输入、视觉识别、语义判断、敏感操作、最终验证 |
...
本技能的前提是"一段机械流程不再为每次点击花掉一个宿主模型轮次"。这值得实测而非断言，因此同一段
3 个动作的任务（展开 → 滚动 → 折叠）在同一测试页上每臂各跑 3 次：

- **A 臂** —— 由宿主 agent 自己驱动浏览器；其工作目录下**看不到**本技能
- **B 臂** —— 宿主 agent 使用本技能

两臂都用宿主运行时自身的 token 计量，正确性取自**页面自报**，而非任一 agent 的自述。

| | A 臂（不用技能） | B 臂（用技能） |
| --- | ---: | ---: |
| 宿主轮次 | 7.7 | 10.3 |
| 未缓存输入 tokens | 42,507 | 41,015 |
| 缓存读取 tokens | 201,259 | 354,347 |
| 输出 tokens | 1,360 | 3,674 |
| **宿主计费成本** | **$0.015592** | **$0.018820** |
| 任务正确完成 | 3/3 | 3/3 |

逐次数据如下，便于你判断离散度而非只看均值：

| 运行 | 轮次 | 未缓存输入 | 缓存读取 | 输出 | 成本 |
| --- | ---: | ---: | ---: | ---: | ---: |
...
**在短且可脚本化的流程上，本技能在每一个样本里都更贵** —— 均值口径贵约 21% —— 且并不更准确：
两臂都是 3/3。我们公布这个结果，而不是挑一个好看的数字。

注意两个分布的形态：B 臂很稳定（$0.0187–$0.0189，波动 ±0.4%），A 臂波动达 2 倍
（$0.0115–$0.0225）。这个技能是用更好的最优情况换取更紧的最差情况。

每臂 3 个样本、单一 3 动作测试页，因此请把百分比视为量级提示，而非精确值。


这个规模下开销占优的原因是：宿主本来就能把 3 个机械动作合并进一两次 `eval` 调用，从来不需要"每次
点击一轮"；而 B 臂需要阅读技能文档并编写接入代码，更长的上下文还放大了缓存读取。

### 什么情况下它才划算

- **长流程，或下一步无法预先知道的流程** —— 宿主只能批量执行它已知的动作，否则必须重新读取页面
  （本实验中每次观察 571–1,711 tokens）
- **宿主模型较贵时** —— 决策工作转移到 Jev，**只对输入计费：$0.042 / 百万 token，输出免费**
  （本流程实测决策成本：输入 4,751 / 输出 211 ≈ $0.0002）
- **安全与可审计**（A/B 未计价）：每步复核 origin 白名单、构造上无文本输入、策略限定控件、强制宿主验证

**盈亏平衡的流程长度尚未测量**。请把它当作待验证的假设，用下面的计量在你自己的流程上检验。
