...
- **实测，而非宣传** — **browser-with-typesafe** 把每个浏览器决策移到 Jev（只对输入计费，$0.042/Mtok）。我们在 15 动作流程上做了与直接驱动的 A/B 对比：**贵 58.9%、每个动作慢 69%**，准确率相同。数字、逐次离散度与复现命令随仓库公布 —— 包括前提不成立的部分。
...
**15 动作流程成本实测** —— 每臂 3 个样本，同一宿主模型、同一目标，正确性取自**页面自报**而非
任一 agent 的自述：
...
本技能**贵 58.9%、每个动作慢 69%**，而准确率相同。**Jev 不是瓶颈** —— 55 次决策，p50 仅
**373 毫秒**。技能赖以成立的前提"每次点击一个宿主轮次"在此并不成立：A 臂约每个动作只需一轮。

途中还修掉了一个真实缺陷：请求载荷过去随已执行步数增长（第 80 步同一决策需 **10,314** 输入 token），
现在已封顶并投影，第 80 步为 **1,929**，且第 10 步之后完全平坦。

完整方法、逐次离散度与复现命令见[指南](browser-with-typesafe-guide-CN.md)。
