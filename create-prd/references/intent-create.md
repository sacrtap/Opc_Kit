...
2. **Autonomously infer all chapter content**: Do not ask chapter by chapter, instead complete the draft inference for all 13 chapters at once based on available information
...
PRD generation follows 5-batch incremental write:
- Batch 1: Ch1-3 → write/create
- Batch 2: Ch4-5 → append
- Batch 3: Ch6-7 → append
- Batch 4: Ch8-10 → append
- Batch 5: Ch11-13 → append

After each batch save:
1. Update `generate_progress` in front matter (checkpoint)
2. Output structured progress notification to user
3. Resume from checkpoint if session interrupted
